          ...
          <div className="flex flex-col items-center">
            ...
            <div className="text-xs text-gray-600 mt-2 bg-gray-50 p-3 rounded-lg max-w-md">
              ...
              <ul className="grid grid-cols-1 gap-y-1">
                <li>
                  <span className="font-medium">rollNumber:</span> String (Required) - Student roll number (e.g., CS21B001)
                </li>
              </ul>
            </div>
          </div>
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
            <h3 className="text-lg font-medium text-gray-800">Preview Students</h3>
            <div className="mt-2 sm:mt-0 text-sm text-gray-600 bg-blue-50 px-3 py-1 rounded-full">{parsedData.length} students will be added</div>
          </div>
        ...
        {step === 2 && (
          <button onClick={handleUpdate} className="px-4 py-2.5 text-sm font-medium text-white bg-[#1360AB] rounded-lg hover:bg-[#0d4a8b] transition-colors shadow-sm flex items-center" disabled={parsedData.length === 0 || isLoading || isUpdating}>
            {isUpdating ? (
              ...
              <>
                <FaCheck className="mr-2" /> Confirm Add
              </>
            )}
          </button>
        )}
