  ...
  const handleDelete = async () => {
    if (await confirm({ message: "Are you sure you want to delete this hostel gate login?", isDestructive: true })) {
      try {
        ...
        toast.error("Failed to delete hostel gate login. Please try again.")
      } finally {
        setIsDeleting(false)
      }
    }
      ...
      <Card>
        <CardHeader style={{ marginBottom: 0 }}>
          <HStack gap="none" align="start" justify="between">
            ...
            <HStack gap={2}>
              <Button onClick={() => setShowEditModal(true)} variant="ghost" size="sm" title="Edit hostel gate login"><Pencil size="1em" /></Button>
              <Button onClick={handleDelete} variant="ghost" size="sm" loading={isDeleting} disabled={isDeleting} title="Delete hostel gate login"><Trash2 size="1em" /></Button>
            </HStack>
          </HStack>
        </CardHeader>
