            ...
            {currentStudents.map((student, index) => (
              <tr key={student.id || index} className={`transition-colors hover:bg-blue-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="flex-shrink-0 h-8 w-8 rounded-full bg-blue-50 flex items-center justify-center">{student.image ? <img src={student.image} alt={student.name} className="h-8 w-8 rounded-full object-cover" /> : <FaUserGraduate className="h-4 w-4 text-[#1360AB]" />}</div>
                    <div className="ml-3">
