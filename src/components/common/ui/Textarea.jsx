 ...
 */
const Textarea = forwardRef(({ name, value, onChange, placeholder, icon, error, disabled = false, readOnly = false, required = false, rows = 4, resize = "vertical", maxLength, showCount = false, id, className = "", style = {}, ...rest }, ref) => {
  ...
  // Build textarea styles using theme variables
  ...
  const countStyles = {
    ...
    color: maxLength && characterCount >= maxLength ? "var(--color-danger)" : "var(--color-text-muted)",
    pointerEvents: "none",
    transition: "var(--transition-colors)",
  }
  ...
  return (
    <div style={containerStyles}>
      ...
      {/* Character Count */}
      {showCount && <span style={countStyles}>{maxLength ? `${characterCount}/${maxLength}` : characterCount}</span>}
    </div>
  )
})
