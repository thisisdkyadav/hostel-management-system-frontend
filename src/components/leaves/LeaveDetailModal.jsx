...
const LeaveDetailModal = ({ leave, onClose, onUpdated, isAdmin }) => {
  ...
  const canModifyStatus = isAdmin && isPending
            ...
            <div className={`p-4 rounded-xl border ${decisionType === "approve" ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-gray-700">{Math.ceil((new Date(leave.endDate) - new Date(leave.startDate)) / (1000 * 60 * 60 * 24))} day(s)</p>
                </div>
              </div>
            </div>
