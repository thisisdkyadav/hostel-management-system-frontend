...
import { Card, Heading, Surface, Text, VStack } from "hzero"

const ComplaintsSummary = ({ complaints = [], loading = false }) => {
  if (loading) {
    return (
      <Card padding="p-3" className="animate-pulse">
    ...
    return (
      <Card padding="p-4">
        ...
        <Text as="div" color="muted" style={{ paddingTop: 'var(--spacing-4)', paddingBottom: 'var(--spacing-4)' }} className="flex flex-col items-center justify-center">
          <AlertCircle size={30} style={{ marginBottom: 'var(--spacing-1)' }} color="var(--color-bg-muted)" />
    ...
    <Card padding="p-0" className="overflow-hidden">
      <Surface padding="var(--spacing-3) var(--spacing-4)" style={{ borderBottom: 'var(--border-1) solid var(--color-border-light)' }} className="flex justify-between items-center">
        <Heading as="h3" weight="medium" color="primary" size="sm" className="flex items-center">
          <ListChecks size={16} style={{ marginRight: 'var(--spacing-1-5)' }} color="var(--color-primary)" />
      ...
      <div style={{ maxHeight: '250px', overflowY: 'auto' }}>
        {complaints.map((complaint) => (
          <div key={complaint.id} className="last:border-0 hover:bg-[var(--color-info-bg-light)]" style={{ padding: 'var(--spacing-2) var(--spacing-4)', borderBottom: 'var(--border-1) solid var(--color-border-light)', transition: 'var(--transition-colors)' }}>
            ...
            <div className="flex justify-between items-center" style={{ marginTop: 'var(--spacing-1)' }}>
              ...
              <Link to={`complaints`} className="transition-colors hover:bg-[var(--color-info-bg)]" style={{ padding: 'var(--spacing-1)', borderRadius: 'var(--radius-full)', color: 'var(--color-primary)' }}>
                <Eye size={12} />
              </Link>
            </div>
          </div>
