    ...
    (detail?.posts || []).length > 0
      ? detail.posts.map((post) => ({
          ...
          requirements: {
            ...
            proposersRequired: post.requirements?.proposersRequired ?? 1,
            secondersRequired: post.requirements?.secondersRequired ?? 1,
...
export const serializeElectionFormForApi = (form) => ({
  ...
  posts: form.posts.map((post) => ({
    ...
    requirements: {
      ...
      proposersRequired: Math.max(1, Number(post.requirements.proposersRequired || 1)),
      secondersRequired: Math.max(1, Number(post.requirements.secondersRequired || 1)),
      requireElectorateMembership: Boolean(post.requirements.requireElectorateMembership),
      requireHostelResident: Boolean(post.requirements.requireHostelResident),
      allowedHostelNames: post.requirements.allowedHostelNames,
