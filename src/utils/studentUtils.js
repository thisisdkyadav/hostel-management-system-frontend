/**
 * Formats dates for API requests
 * @param {Date} date - Date object to format
 * @returns {string} - Formatted date string in YYYY-MM-DD format
 */
export const formatDateForAPI = (date) => {
  if (!date) return null
  const year = date.getFullYear()
  const month = String(date.getMonth() + 1).padStart(2, "0")
  const day = String(date.getDate()).padStart(2, "0")
  return `${year}-${month}-${day}`
}

/**
 * Builds query parameters for student API requests
 * @param {Object} filters - Filter values
 * @param {Object} pagination - Pagination settings
 * @param {Object} sorting - Sorting settings
 * @returns {Object} - Query parameters object for axios
 */
export const buildStudentQueryParams = (filters, pagination, sorting) => {
  const params = {}

  // Add pagination parameters
  params.page = pagination.currentPage
  params.limit = pagination.perPage

  // Handle search term logic
  if (filters.searchTerm) {
    if (filters.searchTerm.match(/^[a-zA-Z\s]+$/)) {
      params.name = filters.searchTerm
    } else if (filters.searchTerm.includes("@")) {
      params.email = filters.searchTerm
    } else {
      params.rollNumber = filters.searchTerm
    }
  }

  // Add filter parameters
  if (filters.department) params.department = filters.department
  if (filters.degree) params.degree = filters.degree
  if (filters.gender) params.gender = filters.gender
  if (filters.hostelId) params.hostelId = filters.hostelId
  if (filters.unitNumber) params.unitNumber = filters.unitNumber
  if (filters.roomNumber) params.roomNumber = filters.roomNumber
  if (filters.yearOfStudy) params.yearOfStudy = filters.yearOfStudy
  if (filters.hasAllocation) params.hasAllocation = filters.hasAllocation
  if (filters.status) params.status = filters.status
  if (filters.isDayScholar !== "") {
    params.isDayScholar = filters.isDayScholar === "true"
  }

  // Add date parameters
  if (filters.admissionDateFrom) {
    params.admissionDateFrom = formatDateForAPI(filters.admissionDateFrom)
  }

  if (filters.admissionDateTo) {
    params.admissionDateTo = formatDateForAPI(filters.admissionDateTo)
  }

  // Add missing options filter
  if (filters.missingOptions && filters.missingOptions.length > 0) {
    params.missing = filters.missingOptions.join(",")
  }

  // Add sorting parameters
  params.sortBy = sorting.sortField
  params.sortOrder = sorting.sortDirection

  return params
}

/**
 * Default filter values
 */
export const DEFAULT_FILTERS = {
  searchTerm: "",
  hostelId: "",
  unitNumber: "",
  yearOfStudy: "",
  department: "",
  degree: "",
  gender: "",
  roomNumber: "",
  hasAllocation: "",
  status: "Active",
  isDayScholar: "",
  admissionDateFrom: null,
  admissionDateTo: null,
  missingOptions: [],
}

/**
 * Default sorting values
 */
export const DEFAULT_SORTING = {
  sortField: "rollNumber",
  sortDirection: "asc",
}
